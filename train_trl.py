"""Train the policy model with LoRA/QLoRA and a learned reward model."""
...
from src.reward_backend import (
    load_reward_backend,
    resolve_reward_backend,
    reward_texts_for_candidates,
    score_candidates_with_llm_reward,
    score_candidates_with_sequence_reward,
)
...
def collect_reward_guided_policy_dataset(
    ...
    backend = load_reward_backend(
        backend=reward_backend,
        reward_model_name=reward_model_name,
        reward_model_path=reward_model_path,
        use_4bit=use_4bit,
    )
    reward_model = backend["model"]
    reward_tokenizer = backend["tokenizer"]
    reward_device = backend["device"]
    env = ESCEnv()
    router = SkillRouter()
    ...
    for task_id in TASKS:
        for _ in range(episodes_per_task):
            ...
            while True:
                ...
                reward_texts = reward_texts_for_candidates(obs, memory, candidates)
                if reward_backend == "llm_judge":
                    scored_candidates = score_candidates_with_llm_reward(
                    ...
                    scored_candidates = score_candidates_with_sequence_reward(
                        reward_model=reward_model,
                        reward_tokenizer=reward_tokenizer,
                        candidates=candidates,
                        reward_texts=reward_texts,
...
def train_model(
    ...
    model, tokenizer, training_backend = build_model_and_tokenizer(
...
def generate_reply(model, tokenizer, prompt: str, max_new_tokens: int = 96) -> str:
    ...
    return first_line or "That sounds really hard. What feels most important to say first?"
...
def write_before_after_md(before: Dict[str, Any], after: Dict[str, Any], output_path: Path) -> None:
    ...
    for ep in before["episodes"]:
        ...
        lines.append(
            f"| {ep['task_id']} | {ep['score']:.3f} | {other['score']:.3f} | {int(ep['success'])} | {int(other['success'])} |"
        )
    lines.append("")
    output_path.write_text("\n".join(lines).strip() + "\n", encoding="utf-8")
def main() -> None:
    parser = argparse.ArgumentParser(description="Train the policy model with LoRA/QLoRA and a learned reward model.")
    parser.add_argument("--model-name", default="Qwen/Qwen2.5-0.5B-Instruct", help="Base policy model name or local path.")
    parser.add_argument("--reward-backend", default="auto", choices=["auto", "none", "regression", "llm_judge"], help="Reward backend selection.")
    parser.add_argument("--reward-model-name", default="", help="Reward model name for llm_judge backend.")
    parser.add_argument("--reward-model-path", default="", help="Path to a trained reward model or backend config directory.")
    ...
    resolved_backend, resolved_model_ref = resolve_reward_backend(
        requested_backend=args.reward_backend,
        reward_model_name=args.reward_model_name,
        reward_model_path=args.reward_model_path,
    )
    if resolved_backend in {"regression", "llm_judge"}:
        samples = collect_reward_guided_policy_dataset(
            episodes_per_task=args.episodes_per_task,
            reward_backend=resolved_backend,
            reward_model_name=resolved_model_ref if resolved_backend == "llm_judge" else "",
            reward_model_path=resolved_model_ref if resolved_backend == "regression" else args.reward_model_path,
            use_4bit=args.use_4bit,
        )
        dataset_source = f"{resolved_backend}_reranked"
    else:
    ...
    eval_dataset = dataset_from_samples(eval_samples, tokenizer_probe)
    before_model, before_tokenizer, _before_backend = build_model_and_tokenizer(
        args.model_name,
        use_4bit=args.use_4bit,
        for_training=False,
    )
